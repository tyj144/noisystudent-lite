...
naes = dset.ImageFolder(root="./datasets/imagenet-a/", transform=test_transform)
...
def get_net_results():
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(nae_loader):
            ...
            # print(target.shape)
            # print(target)
            # show_test_examples(data.permute(0, 2, 3, 1), dim=2)
            ...
            pred = output.data.max(1)[1]
            correct.extend(pred.eq(target).numpy().squeeze().tolist())
            print("Batch:", batch_idx, "Accuracy:",
                  num_correct / len(nae_loader.dataset))
